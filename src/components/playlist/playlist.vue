      ...
      <div class="list-wrapper" @click.stop>
        ...
        <scroll ref="listContent" class="list-content" :data="playList">
          <transition-group ref="list" name="list" tag="ul">
            <li ref="li" class="item border-bottom" v-for="(item, index) of playList" :key="index" @click="selectItem(item, index)">
              <i class="current" :class="getCurrentIcon(item)"></i>
              <span class="text">{{item.name}} - <span style="font-size:10px">{{item.singer}}</span></span>
...
export default {
  ...
  methods: {
    ...
    scrollToCurrent(currentSong) {
      ...
      this.$refs.listContent.scrollToElement(this.$refs.li[index], 300)
    },
    deleteOne(item) {
